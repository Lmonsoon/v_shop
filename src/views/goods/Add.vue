...
import {getparamslist, getadd} from 'network/goods'
export default {
name:'Add',
mixins: [mixingetcatelist],
data () {
    return {
        breadcrumbprop:['商品管理','添加商品'],
        activeIndex:"0",
        //添加商品的表单数据对象
      addForm: {
        goods_name: "",
    ...
    if (!this.isnextfo) {
     ...
     tabClicked() {
      ...
      if (this.activeIndex === "1") {
        getparamslist(this.cateId,{ sel: "many" }).then(resu=>{
            const { data: res}= resu;
        ...
        this.manyTableData = res.data;
            ...
            getparamslist(this.cateId,{ sel: "only" }).then(resu=>{
            const { data: res}= resu;
...
handleRemove(file){
  ...
  //监听图片上传成功的事件
    ...
    add(){
       ...
       data.goods_cat= this.addForm.goods_cat.join(',')
       //处理动态参数
        ...
        data.attrs = this.addForm.attrs;
        getadd(data).then(resu=>{
           
            const { data: res } = resu; console.log(res);
